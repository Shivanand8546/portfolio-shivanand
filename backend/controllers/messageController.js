...
const sendMessage = expressAsyncHandler(async (req, res) => {
    ...
    // senderEmail ya email — dono accept karo
    const senderMail = senderEmail || email;
    
    if (!senderName || !senderMail || !message) {
...
const deleteMessage = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!deletedMessage) {
            ...
            res.status(200).json({ success: true, message: "Message Deleted", deletedMessage });
        }
    } catch (err) {
        throw new Error(err ? err.message : "Something went wrong");
    }
...
module.exports = { sendMessage, getAllMessages, deleteMessage, getSingleMessage };
